...
export const DefinicionColumnToggle: React.FC = () => {
  const { tableColumns, updateTableColumns } = useDefinicionData();
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="inline-flex items-center px-3 py-2 text-sm font-medium text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-300 dark:border-emerald-700 rounded-md hover:bg-emerald-200 dark:hover:bg-emerald-900/50 transition-colors duration-200"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-slate-700 z-50">
          <div className="py-1">
            <div className="px-4 py-2 text-xs font-medium text-emerald-800 dark:text-emerald-200 bg-emerald-50 dark:bg-emerald-900/20">
              Seleccionar columnas visibles
            </div>
            {tableColumns.map((column) => (
              <label
                key={column.key}
                className="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-slate-700 cursor-pointer"
              >
                <input
                  type="checkbox"
                  checked={column.visible}
                  onChange={() => handleToggleColumn(column.key)}
                  className="h-4 w-4 text-emerald-600 focus:ring-emerald-500 border-gray-300 dark:border-slate-600 rounded"
                />
            ...
            <div className="border-t border-gray-200 dark:border-slate-700 mt-2 pt-2">
              <button
                ...
                className="block w-full text-left px-4 py-2 text-sm text-emerald-700 dark:text-emerald-300 hover:bg-emerald-50 dark:hover:bg-slate-700"
              >
                Mostrar todas
              </button>
              <button
