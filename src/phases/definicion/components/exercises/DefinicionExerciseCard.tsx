...
export const DefinicionExerciseCard: React.FC<DefinicionExerciseCardProps> = ({
  exercise,
  index,
  onShowImage,
}) => {
  return (
    <div className="border border-emerald-200 dark:border-slate-700 rounded-lg p-6 bg-gradient-to-r from-white to-emerald-50 dark:from-slate-800 dark:to-slate-800/50">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <span className="flex items-center justify-center w-8 h-8 bg-emerald-600 text-white text-sm font-bold rounded-full">
              {index + 1}
            </span>
        ...
        <button
          ...
          className="ml-4 p-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md transition-colors duration-200"
      ...
      {exercise.alternativas.length > 0 && (
        <div className="mt-6 border-t border-emerald-200 dark:border-slate-700 pt-6">
          ...
          <DefinicionAlternativesList exercise={exercise} onShowImage={onShowImage} />
        </div>
      )}

      {/* Individual Tracker */}
      <div className="mt-6">
        <DefinicionIndividualTracker exercise={exercise} />
      </div>
    </div>
  );
};
