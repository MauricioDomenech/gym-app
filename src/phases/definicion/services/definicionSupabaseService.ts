...
export class DefinicionSupabaseService {
  ...
  public static async getDefinicionBodyComposition(): Promise<DefinicionBodyComposition[]> {
    ...
    return (data || []).map(row => ({
      ...
      musculoEsqueletico: row.musculo_esqueletico,
      masaMuscular: row.masa_muscular,
      aguaCorporal: row.agua_corporal,
      pesoSinGrasa: row.peso_sin_grasa,
      masaOsea: row.masa_osea,
      proteinaCorporal: row.proteina_corporal,
      tmb: row.tmb,
      imc: row.imc,
      cintura: row.cintura,
      cadera: row.cadera,
      ...
      brazo: row.brazo,
      muslo: row.muslo,
      comidasRelax: row.comidas_relax,
      adherenciaNutricion: row.adherencia_nutricion,
      suenoPromedio: row.sueno_promedio,
      hambre: row.hambre,
      energia: row.energia,
  ...
  public static async addDefinicionBodyComposition(entry: DefinicionBodyComposition): Promise<void> {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        grasa_subcutanea: entry.grasaSubcutanea || null,
        grasa_visceral: entry.grasaVisceral || null,
        musculo_esqueletico: entry.musculoEsqueletico || null,
        masa_muscular: entry.masaMuscular || null,
        agua_corporal: entry.aguaCorporal || null,
        peso_sin_grasa: entry.pesoSinGrasa || null,
        masa_osea: entry.masaOsea || null,
        proteina_corporal: entry.proteinaCorporal || null,
        tmb: entry.tmb || null,
        imc: entry.imc || null,
        ...
        comidas_relax: entry.comidasRelax || null,
        adherencia_nutricion: entry.adherenciaNutricion || null,
        sueno_promedio: entry.suenoPromedio || null,
        hambre: entry.hambre || null,
        energia: entry.energia || null,
        molestias: entry.molestias || null,
        notas: entry.notas || '',
      });
